...
cls
pushd "%~dp0"
title Batch HWID Activator for Windows 10/11
mode 76, 30
echo			    Please Select your Edition:
ECHO.
ECHO       		[1] Pro [2] Enterprise [3] Education
ECHO.
SET /P M=Select your Edition and hit the Enter key: 
IF %M%==1 GOTO PRO
IF %M%==2 GOTO ENTERPRISE	
IF %M%==3 GOTO EDUCATION
IF %M%==4 GOTO EOF
IF %M%==exit GOTO EOF
IF %M%==quit GOTO EOF
IF %M%==close GOTO EOF
IF %M%==pissoff GOTO EOF
IF %M%==piss_off GOTO EOF
:PRO
@echo off &chcp 850 >nul &pushd "%~dp0"
@set "0=%~f0" &powershell -nop -c $f=[IO.File]::ReadAllText($env:0)-split':bat2file\:.*';iex($f[1]); X(1) &goto BOTTOM1
...
move gatherosstatemodified.exe C:\HWIDFiles\gatherosstatemodified.exe >NUL
powershell -c (Get-ItemProperty HKLM:\SYSTEM\CurrentControlSet\Control\ProductOptions).OSProductPfn > C:\HWIDFiles\pfn.txt


for /f "delims=" %%x in (C:\HWIDFiles\pfn.txt) do set PFN=%%x
set /p PFN=<C:\HWIDFiles\pfn.txt
echo PFN is %PFN%
ECHO.
C:\HWIDFiles\gatherosstatemodified.exe Pfn=%PFN%;DownlevelGenuineState=1
ECHO.
...
:ENTERPRISE
@echo off &chcp 850 >nul &pushd "%~dp0"
@set "0=%~f0" &powershell -nop -c $f=[IO.File]::ReadAllText($env:0)-split':bat2file\:.*';iex($f[1]); X(1) &goto BOTTOM2
:bat2file: Compressed2TXT v6.5
$k='.,;{-}[+](/)_|^=?O123456789ABCDeFGHyIdJKLMoN0PQRSTYUWXVZabcfghijklmnpqrstuvwxz!@#$&~E<*`%\>'; Add-Type -Ty @'
using System.IO;public class BAT91{public static void Dec(ref string[] f,int x,string fo,string key){unchecked{int n=0,c=255,q=0
,v=91,z=f[x].Length; byte[]b91=new byte[256]; while(c>0) b91[c--]=91; while(c<91) b91[key[c]]=(byte)c++; using (FileStream o=new
...
:EDUCATION
@echo off &chcp 850 >nul &pushd "%~dp0"
@set "0=%~f0" &powershell -nop -c $f=[IO.File]::ReadAllText($env:0)-split':bat2file\:.*';iex($f[1]); X(1) &goto BOTTOM3
:bat2file: Compressed2TXT v6.5
$k='.,;{-}[+](/)_|^=?O123456789ABCDeFGHyIdJKLMoN0PQRSTYUWXVZabcfghijklmnpqrstuvwxz!@#$&~E<*`%\>'; Add-Type -Ty @'
using System.IO;public class BAT91{public static void Dec(ref string[] f,int x,string fo,string key){unchecked{int n=0,c=255,q=0
,v=91,z=f[x].Length; byte[]b91=new byte[256]; while(c>0) b91[c--]=91; while(c<91) b91[key[c]]=(byte)c++; using (FileStream o=new
